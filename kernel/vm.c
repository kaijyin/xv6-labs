...
void
kvmmap(uint64 va, uint64 pa, uint64 size, int perm){
  if(mappages(kernel_pagetable,va,size,pa,perm)!=0){
    panic("kvmap");
  }
}
void
ukvmmap(pagetable_t kpagetable,uint64 va, uint64 pa, uint64 size, int perm){
  if(mappages(kpagetable,va,size,pa,perm)!=0){
    panic("ukvmap");
  }
}
